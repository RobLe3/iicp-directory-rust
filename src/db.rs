...
async fn hash_registration_tokens(rec: &NodeRecord) -> Result<(String, String), RepoError> {
...
async fn upsert_registered_node(
...
async fn replace_registered_capabilities(
    transaction: &mut sqlx::Transaction<'_, MySql>,
    rec: &NodeRecord,
) -> Result<(), RepoError> {
...
async fn persist_registration(
    ...
    if upsert_registered_node(&mut transaction, rec, token_hash, proxy_hash)
        .await
        .is_err()
    {
        return Err(RepoError::Persistence);
...
impl NodeRepository for MySqlRepo {
    ...
    async fn register(&self, rec: NodeRecord) -> Result<(), RepoError> {
        ...
        let (token_hash, proxy_hash) = hash_registration_tokens(&rec).await?;
        persist_registration(&self.pool, &rec, &token_hash, &proxy_hash).await
    }
